...
function createProgram(
  ...
  for (let i = 0; i < 2; i++) {
    const shader = createShader(gl, shaderTypes[i], shaderSources[i]);
...
export default function InfiniteMenu({ items = [], scale = 1.0 }: { items: any[]; scale?: number }) {
  ...
  return (
    <div style={{ position: 'relative', width: '100%', height: '100%', overflow: 'hidden' }}>
      ...
      {activeItem && (
        <div 
          className={`infinite-menu-card ${isMoving ? 'inactive' : 'active'}`}
          onClick={handleButtonClick}
          style={{ cursor: activeItem?.link ? 'pointer' : 'default' }}
        >
          <span className="infinite-menu-category">CERTIFICATION</span>
          <h3 className="infinite-menu-title">{activeItem.title}</h3>
          <p className="infinite-menu-desc" style={{ marginBottom: 0 }}>{activeItem.description}</p>
        </div>
      )}
    </div>
  );
